...
export default function SalesPage() {
  ...
  useEffect(() => {
    ...
    const loadProducts = async () => {
      ...
      const ids = Array.from(new Set(sales.map((o) => o.productId).filter(Boolean)));
  ...
  useEffect(() => {
    ...
    const loadSales = async () => {
      ...
      try {
        ...
        console.error('Failed to load sales for seller', err);
        if (isMounted) {
  ...
  const loading = authLoading || loadingSales;
  ...
  const handleOpenChat = async (order: Order) => {
    ...
    try {
      ...
      console.error('Failed to open conversation for sale', err);
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="flex justify-center items-center h-64">
          ...
          <p className="ml-4 text-slate-300">Se încarcă vânzările tale...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-[60vh] flex items-center justify-center px-4">
        <div className="max-w-xl rounded-2xl border border-[#e7b73c]/40 bg-navy-900/80 p-8 text-center shadow-[0_18px_55px_rgba(0,0,0,0.85)]">
          ...
          <p className="text-sm text-slate-300 mb-5">
            Autentifică-te pentru a vedea vânzările realizate în magazin.
          </p>
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="bg-red-900/50 border border-red-500/50 rounded-2xl p-6 text-center backdrop-blur-sm">
          ...
          <p className="text-red-300">{salesError}</p>
        </div>
      </div>
    );
  }
        ...
        <div>
          ...
          <p className="text-slate-300">
            {isEmpty
              ...
              : `Ai înregistrat ${lines.length} ${
                  lines.length === 1 ? 'vânzare' : 'vânzări'
                } în magazin.`}
          </p>
        </div>
        <Link
      ...
      {isEmpty ? (
        ...
        <div className="space-y-4">
          {lines.map(({ order, product }) => {
            ...
            const buyerLabel = order.buyerName || `Cumpărător #${order.buyerId.slice(-6)}`;
                ...
                <div className="flex-1 flex flex-col justify-between gap-2">
                  <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-1">Cumpărător ID: <span className="font-mono text-slate-300 text-[11px]">{order.buyerId}</span></p>
                    </div>

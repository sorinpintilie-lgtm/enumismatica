...
export default function AuctionWatchlistPage() {
  ...
  // Filter watchlist to only auctions
  const auctionWatchlist = useMemo(() => {
    return watchlist.filter(item => item.itemType === 'auction');
  }, [watchlist]);

  const watchlistWithItems = useMemo(() => {
    return auctionWatchlist.map((item) => {
      const linkedItem = auctions.find((a) => a.id === item.itemId);
      return { ...item, linkedItem };
    });
  }, [auctionWatchlist, auctions]);

  const handleRemove = async (itemId: string) => {
    if (confirm('Ești sigur că vrei să elimini această licitație din lista de urmărire?')) {
      await removeFromWatchlist(itemId);
    }
  };

  const handleClear = async () => {
    if (!auctionWatchlist.length) return;
    if (confirm('Ești sigur că vrei să golești întreaga listă de urmărire a licitațiilor?')) {
      // Only clear auction items from watchlist
      const auctionItems = auctionWatchlist.map(item => item.itemId);
        ...
        <div className="min-h-[60vh] flex items-center justify-center px-4">
          <div className="max-w-xl rounded-2xl border border-[#e7b73c]/40 bg-navy-900/80 p-8 text-center shadow-[0_18px_55px_rgba(0,0,0,0.85)]">
            <h1 className="text-2xl font-bold text-white mb-3">
              Lista de urmărire licitații este disponibilă doar pentru utilizatori autentificați
            </h1>
            <p className="text-sm text-slate-300 mb-5">
              Autentifică-te pentru a accesa lista ta de urmărire a licitațiilor.
            </p>
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-[#e7b73c] mb-1">Licitații urmărite</h1>
          {loading ? (
            <p className="text-slate-300">Se încarcă...</p>
          ) : isEmpty ? (
            <p className="text-slate-300">Nu urmărești nicio licitație.</p>
          ) : (
            <p className="text-slate-300">
              Urmărești {watchlistWithItems.length} {watchlistWithItems.length === 1 ? 'licitație' : 'licitații'}.
            </p>
          )}
        </div>
        <Link
                ...
                return (
                  ...
                  >
                    <div className="text-center text-slate-400">
                      <p className="text-sm">Licitație indisponibilă</p>

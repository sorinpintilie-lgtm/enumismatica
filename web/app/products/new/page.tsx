  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const productRef = await addDoc(collection(db, 'products'), {
        ...
        ngcCode: hasNgcCertification ? ngcCode.trim() : null,
        ngcGrade: hasNgcCertification ? ngcGrade : null,
      ...
      if (hasNgcCertification) {
        if (!ngcCode.trim()) {
          showToast({
            type: 'error',
            title: 'Cod NGC lipsă',
            message: 'Introdu codul NGC pentru certificarea piesei.',
          });
          return;
        }
  
        // Validate NGC code format (should be like "1234567-999")
        const ngcCodeRegex = /^\d{7}-\d{3}$/;
        if (!ngcCodeRegex.test(ngcCode.trim())) {
          showToast({
            type: 'error',
            title: 'Format cod NGC invalid',
            message: 'Codul NGC trebuie să fie în formatul XXXXXXX-YYY (ex: 1234567-999).',
          });
          return;
        }
        ...
        if (!ngcGrade) {
          showToast({
            ...
            message: 'Selectează gradul NGC pentru certificarea piesei.',
          });
          return;
        }
      }
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="mb-6 p-4 bg-gradient-to-r from-blue-500/10 to-blue-600/10 rounded-lg border border-blue-500/30">
            <div className="flex items-center mb-4">
              <input
                type="checkbox"
                id="ngc-certification"
                checked={hasNgcCertification}
                onChange={(e) => {
                  setHasNgcCertification(e.target.checked);
                  if (!e.target.checked) {
                    setNgcCode('');
                    setNgcGrade('');
                  }
              ...
              />
              <label htmlFor="ngc-certification" className="ml-3 text-sm font-semibold text-blue-400">
                Am certificare NGC
              </label>
            </div>
            <p className="text-xs text-slate-400 mb-4">
              Marchează această opțiune dacă piesa este certificată de NGC (Numismatic Guaranty Corporation).
              Piesele NGC sunt încapsulate într-un dreptunghi și au un cod unic de certificare.
            </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="text"
                    value={ngcCode}
                    onChange={(e) => setNgcCode(e.target.value)}
                ...
                <div>
                  ...
                  <select
                    value={ngcGrade}
                    onChange={(e) => setNgcGrade(e.target.value)}
